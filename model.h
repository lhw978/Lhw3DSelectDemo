...
public:
	...
	Model(std::string path, bool gamma = false) : gammaCorrection(gamma)//构造函数
	{
	...
	void Draw(Shader shader)//遍历每个网格并调用他们自己的draw函数
	{
...
private:
	...
	void loadModel(std::string const& path)//加载文件
	{
		// read file via ASSIMP
		Assimp::Importer importer;//声明了Assimp命名空间内的一个Importer
		const aiScene* scene = importer.ReadFile(path, aiProcess_Triangulate | aiProcess_FlipUVs | aiProcess_CalcTangentSpace);//把文件按照一定规则加载给scane
		// check for errors
		if (!scene || scene->mFlags & AI_SCENE_FLAGS_INCOMPLETE || !scene->mRootNode) // if is Not Zero//检测数据是否完整
		{
			std::cout << "ERROR::ASSIMP:: " << importer.GetErrorString() << std::endl;
			return;
		}
		// retrieve the directory path of the filepath
		directory = path.substr(0, path.find_last_of('/'));//这里实际上就是获取到了/之前的那段路径   但是我目前觉得应该是//

		// process ASSIMP's root node recursively
		processNode(scene->mRootNode, scene);//将第一个节点（根节点）传入  //这是一个递归函数 不停的调用所有函数 直到全部调用完
	}
	...
	Mesh processMesh(aiMesh* mesh, const aiScene* scene)//处理网格函数 也就是在这里对每个网格开始处理他的数据
	{
		...
		for (unsigned int i = 0; i < mesh->mNumVertices; i++)//mesh->mNumVertices网格的总数量
		{
			Vertex vertex;
			glm::vec3 vector; // we declare a placeholder vector since assimp uses its own vector class that doesn't directly convert to glm's vec3 class so we transfer the data to this placeholder glm::vec3 first.
			//这里开始获取通过网格获取网格的顶点 法线
			// positions
